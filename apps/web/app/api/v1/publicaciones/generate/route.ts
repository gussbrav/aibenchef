...
export async function POST(req: NextRequest) {
  return handleRoute(async () => {
    ...
    // Admins bypasean todos los limites de plan (sin restriccion de peers
    // ni de publicaciones por mes — son los operadores del sistema).
    const userIsAdmin = session.role === "admin" || await isAdmin(session.id);
    ...
    if (!userIsAdmin) {
      ...
      // Se chequea ANTES de gastar tokens del LLM. UI ya deberia advertir,
