...
/**
 * Delete button para tarjeta de notebook en la grilla. Mismo patron que
 * DeleteTableroButton — sobre la tarjeta como Link contenedor, visible al
 * hover, intercepta el click para no navegar.
 */
export function DeleteNotebookButton({ id, titulo }: { id: string; titulo: string }) {
  ...
  const onClick = async (e: React.MouseEvent) => {
    ...
    const ok = await confirm({
      title: `Eliminar notebook "${titulo}"`,
      message:
        "Vas a eliminar el notebook y todas sus celdas. Esta accion no se puede deshacer.",
    ...
    setErrMsg(null);
    try {
      const r = await fetch(`/api/v1/notebooks/${id}`, { method: "DELETE" });
