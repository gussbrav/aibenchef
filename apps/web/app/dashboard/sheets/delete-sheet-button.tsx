...
/**
 * Delete button para tarjeta de sheet en la grilla. Mismo patron que
 * DeleteTableroButton — sobre la tarjeta como Link contenedor, visible al
 * hover, intercepta el click para no navegar.
 */
export function DeleteSheetButton({ id, nombre }: { id: string; nombre: string }) {
  ...
  const onClick = async (e: React.MouseEvent) => {
    ...
    const ok = await confirm({
      title: `Eliminar sheet "${nombre}"`,
      message:
        "Vas a eliminar la hoja y todas sus celdas. Esta accion no se puede deshacer.",
    ...
    setErrMsg(null);
    try {
      const r = await fetch(`/api/v1/sheets/${id}`, { method: "DELETE" });
